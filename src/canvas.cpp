...
void Lab5Canvas::subdivideSquare(GridPoint topleft, GridPoint botright, GLint curDepth)
{
    ...
    Vector3 vTM = (vTL + vTR) / 2;
    Vector3 vMR = (vTR + vBR) / 2;
    Vector3 vBM = (vBL + vBR) / 2;
    Vector3 vML = (vTL + vBL) / 2;
    Vector3 vMM_unperturbed = (vTM + vMR + vBM + vML) / 4;

    Vector3 vMM = Vector3(vMM_unperturbed.x, vMM_unperturbed.y + getPerturb(curDepth), vMM_unperturbed.z);

    GridPoint TM = GridPoint((TL.x + TR.x) / 2, (TL.y + TR.y) / 2);
    GridPoint MR = GridPoint((TR.x + BR.x) / 2, (TR.y + BR.y) / 2);
    GridPoint BM = GridPoint((BL.x + BR.x) / 2, (BL.y + BR.y) / 2);
    GridPoint ML = GridPoint((TL.x + BL.x) / 2, (TL.y + BL.y) / 2);
    GridPoint MM = GridPoint(TM.x, ML.y);

    // enter results into the terrain array
...
void Lab5Canvas::computeNormals()
{
    ...
    for (int row = 0; row < m_gridLength; row++)
    {
        for (int column = 0; column < m_gridLength; column++)
        {
            ...
            for (int i = 0; i < numNeighbors; ++i)
            {
                int next = (i + 1) % numNeighbors;
                normals[i] = offsets[i].cross(offsets[next]);
            }
...
void Lab5Canvas::paintGL()
{
    ...
    for (int row = 0; row < (m_gridLength - 1); row++)
    {
        ...
        {
            GridPoint gridPosition(row, col);
            GridPoint nextPosition(row+1, col);

            Vector3 &v1 = m_terrain[getIndex(gridPosition)];
            Vector3 &v2 = m_terrain[getIndex(nextPosition)];

            Vector3 &normal1 = m_normalMap[getIndex(gridPosition)];
            Vector3 &normal2 = m_normalMap[getIndex(nextPosition)];

            glNormal3dv(normal1.data);
            glTexCoord2f(row / (float) m_gridLength, col / (float) m_gridLength);
            glVertex3dv(v1.data);

            glNormal3dv(normal2.data);
            glTexCoord2f((row + 1) / (float) m_gridLength, col / (float) m_gridLength);
            glVertex3dv(v2.data);
        }
